...
import ltd.qubit.commons.dao.mapper.StateUpdatableByParentNameMapper;
 ...
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 * <li>{@link #updateStateByName(Long, String, State)}</li>
 * </ul>
 *
 * @param <T>
 ...
 * @author 胡海星
 */
public interface StateUpdatableByParentNameDao<T extends WithName & Stateful>
    extends Dao<T> {

  @Override
  StateUpdatableByParentNameMapper<T> getMapper();
  ...
  default Instant updateStateByName(final Long parentId, final String name,
      final State state) throws DataAccessException {
    return DaoImplHelper.updatePropertyByKeyImpl(this,
        modifyTime -> getMapper().updateStateByName(parentId, name, state, modifyTime),
        "state", state, "name", name, new KeyValuePair("parentId", parentId));
  }
}
