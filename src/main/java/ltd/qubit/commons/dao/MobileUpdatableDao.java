...
/**
 * 此接口表示对有手机号码的实体类实现了更新手机号码操作的DAO。
 *
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 ...
 *
 * @param <T>
 *     被操作的实体的类型，必须实现{@link WithMobile}接口。
 * @author 胡海星
 */
public interface MobileUpdatableDao<T extends Identifiable & WithMobile>
    extends Dao<T> {

  @Override
  MobileUpdatableMapper<T> getMapper();
   ...
   *     如果发生其他无法归类的数据库操作错误。
   */
  @Modified({"mobile", "modifyTime"})
  default Instant updateMobile(final Long id, final Phone mobile)
      throws DataAccessException {
    return updatePropertyByKeyImpl(this,
        modifyTime -> getMapper().updateMobile(id, mobile, modifyTime),
        "mobile", mobile, "id", id);
  }
}
