...
import ltd.qubit.commons.dao.mapper.UpdatableByParentCodeMapper;
 ...
 * @author 胡海星
 */
public interface UpdatableByParentCodeDao<T extends WithCode> extends Dao<T> {

  @Override
  UpdatableByParentCodeMapper<T> getMapper();

  /**
   * 根据编码更新一个已存在的实体。
   *
   * @param obj
   *     待更新的已存在的实体对象的新数据，更新时使用其<b>所属父对象的ID</b>及其<b>编码</b>
   *     确定该对象。
   * @return
   *     数据被修改时的时间戳。
   ...
   *     如果发生其他无法归类的数据库操作错误。
   */
  @Unmodified({"id", "code", "entity", "app", "owner", "username",
      "password", "visible", "enabled", "state", "creator", "createTime",
      "deleter", "deleteTime"})
  default Instant updateByCode(final T obj) throws DataAccessException {
    return DaoImplHelper.updateByKeyImpl(this, modifyTime -> getMapper().updateByCode(obj),
