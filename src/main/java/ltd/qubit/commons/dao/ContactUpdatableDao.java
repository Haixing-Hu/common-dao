...
/**
 * 此接口表示对有联系方式的实体类实现了更新联系方式操作的DAO。
 *
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 * <li>{@link #updateContact(Long, Contact)}</li>
 * </ul>
 *
 * @param <T>
 ...
 * @author 胡海星
 */
public interface ContactUpdatableDao<T extends Identifiable & WithContact>
    extends Dao<T> {

  @Override
  ContactUpdatableMapper<T> getMapper();
  ...
  default Instant updateContact(final Long id, @Nullable final Contact contact)
      throws DataAccessException {
    return DaoImplHelper.updatePropertyByKeyImpl(this,
        modifyTime -> getMapper().updateContact(id, contact, modifyTime),
