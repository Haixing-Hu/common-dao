...
/**
 * 此接口表示对有地理位置坐标的实体类实现了更新地理位置坐标操作的DAO。
 *
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 * <li>{@link #updateLocation(Long, Location)}</li>
 * </ul>
 *
 * @param <T>
 ...
 * @author 胡海星
 */
public interface LocationUpdatableDao<T extends Identifiable & WithLocation>
    extends Dao<T> {

  @Override
  LocationUpdatableMapper<T> getMapper();
  ...
  default Instant updateLocation(final Long id, @Nullable final Location location)
      throws DataAccessException {
    return updatePropertyByKeyImpl(this,
        modifyTime -> getMapper().updateLocation(id, location, modifyTime),
