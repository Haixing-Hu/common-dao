...
import ltd.qubit.commons.model.WithComment;

/**
 * 此接口表示对有备注和全局唯一编码的实体类实现了更新备注操作的DAO。
 *
 * <p><b>注意：</b>对象的编码必须全局唯一。</p>
 *
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 * <li>{@link #updateCommentByCode(String, String)}</li>
 * </ul>
 *
 * @param <T>
 *     被操作的实体的类型，必须实现{@link WithComment}接口。
 * @author 胡海星
 */
public interface CommentUpdatableByCodeDao<T extends WithCode & WithComment>
    extends Dao<T> {

  @Override
  CommentUpdatableByCodeMapper<T> getMapper();
   ...
   *     如果发生其他无法归类的数据库操作错误。
   */
  @Modified({"comment", "modifyTime"})
  default Instant updateCommentByCode(final String code, @Nullable final String comment)
      throws DataAccessException {
    return DaoImplHelper.updatePropertyByKeyImpl(this,
        modifyTime -> getMapper().updateCommentByCode(code, comment, modifyTime),
