...
/**
 * 此接口表示实现根据所有者更新实体操作的DAO。
 *
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 * <li>{@link #updateByOwner(T)}</li>
 * </ul>
 *
 * @param <T>
 *     被操作的实体的类型。
 * @author 胡海星
 */
public interface UpdatableByOwnerDao<T extends WithOwner>
    extends Dao<T> {

  @Override
  UpdatableByOwnerMapper<T> getMapper();

  /**
   * 根据所有者更新一个已存在的实体。
   *
   * @param obj
   *     待更新的已存在的实体对象的新数据，更新时使用其<b>所有者</b>确定该对象。
   * @return
   *     数据被修改时的时间戳。
   ...
   *     如果发生其他无法归类的数据库操作错误。
   */
  @Unmodified({"id", "code", "entity", "name", "app", "owner", "username",
      "password", "visible", "enabled", "state", "creator", "createTime",
      "deleter", "deleteTime"})
  default Instant updateByOwner(final T obj) throws DataAccessException {
    return DaoImplHelper.updateByKeyImpl(this, modifyTime -> getMapper().updateByOwner(obj),
