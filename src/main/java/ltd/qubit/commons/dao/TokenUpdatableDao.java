...
/**
 * 此接口表示对有令牌的实体类实现了更新状态操作的DAO。
 *
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 * <li>{@link #updateToken(Long, Token)}</li>
 * </ul>
 *
 * @param <T>
 ...
 * @author 胡海星
 */
public interface TokenUpdatableDao<T extends Identifiable & WithToken>
    extends Dao<T> {

  @Override
  TokenUpdatableMapper<T> getMapper();
  ...
  default Instant updateToken(final Long id, @Nullable final Token token)
      throws DataAccessException {
    return updatePropertyByKeyImpl(this,
        modifyTime -> getMapper().updateToken(id, token, modifyTime),
        "token", token, "id", id);
  }
}
