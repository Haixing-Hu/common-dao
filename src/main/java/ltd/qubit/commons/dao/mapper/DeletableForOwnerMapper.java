...
/**
 * 此接口表示实现删除指定所有者所属实体操作的MyBatis Mapper。
 *
 ...
 * @author 胡海星
 */
public interface DeletableForOwnerMapper<T extends Identifiable & WithOwner & Deletable>
    extends Mapper<T> {
   ...
   */
  long deleteForOwner(@Param("owner") Owner owner,
   ...
   * @throws DataAccessException
   *     若发生任何其他数据存取错误。
   * @see #deleteForOwner(Owner, Instant)
   * @see #purgeForOwner(Owner)
   */
  long restoreForOwner(@Param("owner") Owner owner) throws DataAccessException;
   ...
   * @see #restoreForOwner(Owner)
   */
  long purgeForOwner(@Param("owner") Owner owner) throws DataAccessException;
}
