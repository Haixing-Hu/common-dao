...
/**
 * 此接口表示对拥有所属父对象下唯一编码的实体类实现了查询操作的MyBatis Mapper。
 *
 ...
 * @author 胡海星
 */
public interface GettableByParentCodeMapper<T extends WithCode> extends Mapper<T> {
   ...
   *     如果出现任何数据存取错误。
   */
  boolean existCode(@Param("parentCode") String parentCode,
   ...
   */
  @Nullable
  T getByCode(@Param("parentCode") String parentCode,
      @Param("code") String code) throws DataAccessException;
   ...
   */
  @Nullable
  Long getIdByCode(@Param("parentCode") String parentCode,
      @Param("code") String code) throws DataAccessException;
}
