...
/**
 * 此接口表示对有电子邮件地址的实体类实现了更新电子邮件地址操作的MyBatis Mapper。
 *
 ...
 * @author 胡海星
 */
public interface EmailUpdatableMapper<T extends Identifiable & WithEmail>
    extends Mapper<T> {
   ...
   *     如果发生其他无法归类的数据库操作错误。
   */
  long updateEmail(@Param("id") Long id, @Param("email") String email,
