...
/**
 * 此接口表示对拥有{@code mobile}属性的实体类实现了查询操作的MyBatis Mapper。
 *
 ...
 * @author 胡海星
 */
public interface GettableByMobileMapper<T extends WithMobile> extends Mapper<T> {
   ...
   *     如果出现任何数据存取错误。
   */
  boolean existMobile(@Param("mobile") Phone mobile) throws DataAccessException;
   ...
   */
  @Nullable
  T getByMobile(@Param("mobile") Phone mobile) throws DataAccessException;
   ...
   */
  @Nullable
  Long getIdByMobile(@Param("mobile") Phone mobile) throws DataAccessException;
}
