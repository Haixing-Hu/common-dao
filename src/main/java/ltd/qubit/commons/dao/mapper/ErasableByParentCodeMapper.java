...
/**
 * 此接口表示实现根据所属App及其编码彻底删除实体操作的MyBatis Mapper。
 *
 ...
 * @author 胡海星
 */
public interface ErasableByParentCodeMapper<T extends WithCode>
    extends Mapper<T> {
   ...
   * @see DeletableByParentCodeMapper#deleteByCode(Long, String, Instant)
   * @see DeletableMapper#restore(Long)
   * @see DeletableByParentCodeMapper#restoreByCode(Long, String)
   * @see DeletableMapper#purge(Long)
   * @see DeletableByParentCodeMapper#purgeByCode(Long, String)
   * @see DeletableMapper#purgeAll()
   */
  long eraseByCode(@Param("parentId") Long parentId,
