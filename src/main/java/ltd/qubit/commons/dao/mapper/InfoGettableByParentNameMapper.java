...
/**
 * 此接口表示对拥有{@code info}属性和父对象下唯一名称的实体类实现了查询操作的MyBatis Mapper。
 *
 ...
 * @author 胡海星
 */
public interface InfoGettableByParentNameMapper<T extends HasInfo> extends Mapper<T> {
   ...
   *     指定的对象的名称。
   * @return
   *     在指定的父对象下，具有指定名称的对象的基本信息，或{@code null}若指定的对象不存在。
   * @throws DataAccessException
   *     若出现其他数据存取错误。
   */
  @Nullable
  Info getInfoByName(@Param("parentCode") String parentCode,
