...
/**
 * 此接口表示实现根据编码标记删除实体操作的MyBatis Mapper。
 *
 ...
 * @author 胡海星
 */
public interface DeletableByCodeMapper<T extends Identifiable & WithCode & Deletable>
    extends Mapper<T> {
   ...
   * @throws DataAccessException
   *     若发生任何其他数据存取错误。
   * @see #deleteByCode(String, Instant)
   * @see #purgeByCode(String)
   */
  long restoreByCode(@Param("code") String code) throws DataAccessException;
   ...
   * @see #restoreByCode(String)
   */
  long purgeByCode(@Param("code") String code) throws DataAccessException;
}
