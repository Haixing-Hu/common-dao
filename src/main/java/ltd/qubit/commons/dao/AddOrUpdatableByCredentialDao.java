...
import ltd.qubit.commons.dao.mapper.AddOrUpdatableByCredentialMapper;
...
import static ltd.qubit.commons.dao.DaoImplHelper.addOrUpdateByCredentialImpl;

/**
 * 此接口表示实现根据证件添加或更新实体操作的DAO。
 *
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 * <li>{@link #existCredential(CredentialInfo)}
 * <li>{@link #getByCredential(CredentialInfo)}</li>
 * <li>{@link #add(Object)}</li>
 * <li>{@link #update(Identifiable)}</li>
 * <li>{@link #addOrUpdateByCredential(T)}</li>
 * </ul>
 *
 * @param <T>
 *     被操作的实体的类型。
 * @author 胡海星
 */
public interface AddOrUpdatableByCredentialDao<T extends Identifiable & WithCredential>
    extends GettableByCredentialDao<T>, AddableDao<T>, UpdatableByCredentialDao<T> {

  @Override
  AddOrUpdatableByCredentialMapper<T> getMapper();
   ...
   *     若发生任何其他数据存取错误。
   */
  default Instant addOrUpdateByCredential(final T obj) throws DataAccessException {
    return addOrUpdateByCredentialImpl(this, obj);
  }
