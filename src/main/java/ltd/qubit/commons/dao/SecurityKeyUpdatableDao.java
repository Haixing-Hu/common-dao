...
/**
 * 此接口表示对有安全密钥的实体类实现了更新状态操作的DAO。
 *
 * <p>此接口实现了以下DAO操作：</p>
 * <ul>
 * <li>{@link #updateSecurityKey(Long, String)}</li>
 * </ul>
 *
 * @param <T>
 ...
 * @author 胡海星
 */
public interface SecurityKeyUpdatableDao<T extends Identifiable & WithSecurityKey>
    extends Dao<T> {

  @Override
  SecurityKeyUpdatableMapper<T> getMapper();
  ...
  default Instant updateSecurityKey(final Long id, final String securityKey)
      throws DataAccessException {
    return updatePropertyByKeyImpl(this,
        modifyTime -> getMapper().updateSecurityKey(id, securityKey, modifyTime),
        "securityKey", securityKey, "id", id);
  }
}
