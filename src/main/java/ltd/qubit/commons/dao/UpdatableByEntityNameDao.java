...
import ltd.qubit.commons.dao.mapper.UpdatableByEntityNameMapper;
...
/**
 * 此接口表示实现根据{@code entity}和{@code name}属性更新实体操作的DAO。
 *
 ...
 * @author 胡海星
 */
public interface UpdatableByEntityNameDao<T extends WithEntity & WithName>
    extends Dao<T> {

  @Override
  UpdatableByEntityNameMapper<T> getMapper();
   ...
   *     如果发生其他无法归类的数据库操作错误。
   */
  @Unmodified({"id", "code", "entity", "name", "app", "owner",
      "username", "password", "visible", "enabled", "state", "creator",
      "createTime", "deleter", "deleteTime"})
  default Instant updateByName(final T obj) throws DataAccessException {
    return updateByKeyImpl(this, modifyTime -> getMapper().updateByName(obj),
        obj, "name", obj.getName(), new KeyValuePair("entity", obj.getEntity()));
  }
}
